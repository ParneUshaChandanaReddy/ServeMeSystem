...
public class CustomerServicesFragment extends Fragment {
    ...
    CustomerServicesAdapter mAdapter;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {

        View view = inflater.inflate(R.layout.customer_services_fragment, container, false);
        rViewCustomerServices = view.findViewById(R.id.rViewCustomerServices);
        servicList = new ArrayList<>();

        DatabaseReference dR = FirebaseDatabase.getInstance().getReference(customerServiceRequest).child(loggedInUser.getPhoneNo());
    ...
    private void setAdapterr() {
        mAdapter = new CustomerServicesAdapter(this);
